...
char p[] =
    { 0x70, 0x3A, 0xC1, 0x6F, 0x9A, 0x92, 0xE5, 0x29, 0x18, 0xE7, 0x9E, 0x50,
    0xD6, 0xA5, 0x8D, 0xCC,
...
char pexp[] =
    { 0x1C, 0x0E, 0xB0, 0x5B, 0xE6, 0xA4, 0xB9, 0x4A, 0x46, 0x39, 0xE7, 0x94,
    0x35, 0xA9, 0x63, 0x73,
...
char q[] =
    { 0x16, 0x56, 0x15, 0x79, 0x9A, 0x53, 0x60, 0x4F, 0x87, 0x55, 0x9F, 0xB2,
    0x78, 0x72, 0xE5, 0xBC,
...
char qexp[] =
    { 0x05, 0x95, 0x85, 0x5E, 0x66, 0x94, 0xD8, 0x13, 0xE1, 0xD5, 0x67, 0xEC,
    0x9E, 0x1C, 0xB9, 0x6F,
...
unsigned char n[] =
    { 0x09, 0xCA, 0xC9, 0xC6, 0xB4, 0x12, 0x08, 0x1B, 0x60, 0x58, 0x81, 0x4B,
    0x86, 0x01, 0xD8, 0xBF,
...
char ap[] =
    { 0x04, 0x81, 0xBE, 0x5B, 0x4C, 0x70, 0x0F, 0xBC, 0x42, 0xA2, 0xCD, 0x8C,
    0xE7, 0x00, 0x26, 0x56,
...
char aq[] =
    { 0x05, 0x49, 0x0B, 0x6B, 0x67, 0xA1, 0xF8, 0x5F, 0x1D, 0xB5, 0xB3, 0xBE,
    0x9F, 0x01, 0xB2, 0x69,
...
void usage (void)
{
    ...
    fprintf (stderr,
	     "    -t             test existing cartridge signature only\n");
    fprintf (stderr,
	     "    -w             write back update signature to cartridge\n");
    exit (1);
}
...
int main (int argc, char *const argv[])
{
    ...
    {
	for (i = 0;
	     i < 0x78 && ((cart[0xFF80 + i] == 0)
			  || (cart[0xFF80 + i] == 0xff)); i++);
	if (i == 0x78)
	...
	if (encrypt_hash ())
	{
		...
		{
		    printf
			("Cartridge already has a valid signature, not re-writing file.");
		}
		else
		{
		    f = fopen (fname, "rb+");
