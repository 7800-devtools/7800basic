; --------------------------------------------------------------------------
; UpdateObjects4 assembly code provided by RevEng (Mike Saarna)
; (Use this assembly file if you're using 4 byte sprites. If not, then use)
; the UpdateObjects assembly file instead.)   
;   
...
; to 40 objects in the zone. Extend the x4table if you need more objects, 
...
; -------------------------------------------------------------------------- 
 ...
 jsr SetObjectPointer
 ldy #1 ; palette+width is the 2nd byte in the dl object
 lda (temp1),y
 and #%00011111
 ldx {1}
 ora x32table,x
 ...
 jsr SetObjectPointer
 ldy #3 ; object x-coordinate is the 4th byte in the dl object
 lda {1}
 sta (temp1),y
 ENDM
...
SetObjectPointer ; called with X=object_X Y=object_Y
 ...
 ifconst DOUBLEBUFFER
     ...
     ; we should be safe from overflow despite the 2 adc's.^M
     ; a page full of objects is 51 objects, which is too many to render.^M
 endif ; DOUBLEBUFFER
 sta temp1 
 lda ZONESTARTHI,y
 adc #0
...
x4table ; enough entries to support 40 objects per dl
MULT4IDX SET 0
 REPEAT 40
 .byte MULT4IDX
MULT4IDX SET (MULT4IDX+4)
 REPEND
